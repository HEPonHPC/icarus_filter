...
class art::ProductSizeFilter : public SharedFilter {
  public:
    struct Config {
      ...
      Atom<size_t> maxSizeParicles{Name("maxSizeParticles"),500000000};
    };
...
ProductSizeFilter::ProductSizeFilter(Parameters const& config, ProcessingFrame const&)
  ...
  , max_size_Channels{config().maxSizeChannels()}
  , max_size_Particles{config().maxSizeParticles()}
{
  async<InEvent>();
}

  bool
void ProductSizeFilter::filter(art::Event& e)
{
  art::Handle<sim:SimPhotons> h_photons;
  art::Handle<sim:SimChannel> h_channels;
  art::Handle<simb:MCParticle> h_particles;
...
DEFINE_ART_MODULE(ProductSizeFilter)
